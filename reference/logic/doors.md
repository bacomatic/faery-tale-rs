...
- **`doorfind` vs. `check_door` — two entirely different systems.** This is the most important distinction for visible doors:
  - `check_door` (Phase 12, `game-loop.md`) — fires every frame, matches `doorlist[]` by world coordinates `(xc1, yc1)`, does NOT look at terrain codes. The outdoor tile at a building entrance (HWOOD, VWOOD, HSTONE…) is passable terrain — the hero walks into it and `check_door` detects the coordinate overlap and calls `xfer`. No `doorfind` is involved.
  - `doorfind` / `open_list[]` — fires only when `proxcheck` returns 15 (terrain class 15 = TERRAIN_DOOR, a tile that blocks movement). These are intra-region doors (interior doors within buildings or dungeons). The hero cannot walk through them; bumping causes `doorfind` to try to open the tile. A failed open (locked) does NOT block `check_door` — `check_door` operates independently.
  - In short: **exterior building entrances use `doorlist`+`check_door`** (coordinate match). **Interior locked/unlocked doors use `open_list`+`doorfind`** (tile terrain match). They are orthogonal.
- **`TERRAIN_DOOR` inline handler.** The one-line collision trigger at `fmain.c:1607` — `if (i==0 && j==15) doorfind(xtest,ytest,0);` — is part of [`walk_step`](movement.md#walk_step). It fires `doorfind` with `keytype=0` whenever the hero's proposed next tile has terrain class 15, giving the "bump to open unlocked wood doors" behaviour. `bumped` is cleared on the next step where `j != 15`. Note: `j==15` causes the hero NOT to move (blocked), so the tile opens but the hero stands in place; on the next frame the tile is no longer terrain-15 and movement proceeds.
- **`open_list` NOKEY entries are not exterior doors.** `open_list` entries with `keytype=NOKEY` (idx 1 HWOOD, idx 2 VWOOD, idx 13 HSTON3, idx 14 VSTON3, idx 16 BLUE) are interior doors within building or dungeon maps that happen to have no key requirement. They look like doors (terrain-15 tiles), block movement, and open on bump. They are NOT the same as the exterior wood door graphic that the hero stands in front of when entering a building — that exterior tile is not terrain-15.
