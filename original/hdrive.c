...
int AllocDiskIO() {
    ...
    if (lock = Lock("image", ACCESS_READ)) {
        ...
        if ((file = Open("image", MODE_OLDFILE)) == NULL)
            return 30;
    } else {
        if ((diskport = CreatePort(0, 0)) == 0)
            return 30;
        SETFN(AL_PORT);
        if ((diskreq1 = (struct IOExtTD *)CreateExtIO(
                 diskport, sizeof(struct IOExtTD))) == 0)
            return 31;
        SETFN(AL_IOREQ);
...
int CheckDiskIO(int num) {
    if (hdrive == FALSE)
        return CheckIO((struct IORequest *)&diskreqs[num]);
...
int CheckLastDiskIO() {
    if (hdrive == FALSE)
        return CheckIO((struct IORequest *)lastreq);
