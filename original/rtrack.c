...
int main() {
    ...
    error = OpenDevice(TD_NAME, 1, (struct IORequest *)diskreq, 0);
    if (error)
        goto exit_ior;
...
#ifdef blarg
save_track_range(first_block, block_count, buffer) short first_block,
    block_count;
char *buffer;
{
    ...
    while (first_block < last_block) {
        ...
        if (trackend_block >= last_block)
            ...
            read_length = trackend_block - first_block;
