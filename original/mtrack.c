...
struct cmap {
    char *filename;
    short block_start, block_count;
} char_map[CHAR_ENTRIES] = {
    {"dh0:z/julian", 1376, 42},  {"dh0:z/phillip", 1418, 42},
    {"dh0:z/kevin", 1460, 42},   {"dh0:z/objects", 1312, 36},
    {"dh0:z/raft", 1348, 3},

    {"dh0:z/turtle", 1351, 20},  {"dh0:z/bird", 1120, 40},
    {"dh0:z/dragon", 1160, 12},  {"dh0:z/ogre", 960, 40},
    {"dh0:z/ghost", 1080, 40},

    {"dh0:z/dKnight", 1000, 40}, {"dh0:z/spirit", 1040, 40},
    {"dh0:z/royal", 931, 5},     {"dh0:z/wizard", 936, 5},
    {"dh0:z/bartender", 941, 5},

    {"dh0:z/witch", 946, 5},     {"dh0:z/beggar", 951, 5}};
...
int main() {
    ...
    for (i = 0; i < MAP_ENTRIES; i++) {
        transfer_file(diskmap[i].filename, diskmap[i].block_start,
                      diskmap[i].block_count);
    }

    for (i = 0; i < CHAR_ENTRIES; i++) {
        transfer_char_file(char_map[i].filename, char_map[i].block_start,
...
void transfer_file(char *name, short start, long length) {
    long file;
    short blocks;
    file = Open(name, 1005);
    if (file) {
...
void transfer_char_file(char *name, short start, long length) {
    long file;
    short blocks;
    file = Open(name, 1005);
    if (file) {
...
void save_track_range(short first_block, short block_count, char *buffer) {
    short error, i;
    long bit, off, mask;
...
void read_block(short block, APTR buffer) {
    short error;
...
void write_block(short block, APTR buffer) {
    short error;
    diskreq->iotd_Req.io_Length = 512;
    diskreq->iotd_Req.io_Data = buffer;
    diskreq->iotd_Req.io_Command = CMD_WRITE;
...
ULONG checksum(struct Block *block) {
    short i;
    ULONG sum = 0;
    for (i = 0; i < 128; i++)
        sum -= block->offset[i];
