...
void main(int argc, char *argv[]) {
    ...
    if (!(cdport = CreatePort(NULL, 0)))
        closeall("Can't allocate port");
    if (!(cdreq = CreateStdIO(cdport)))
        closeall("Can't allocate ioreq");
    ...
    if (file == NULL)
        closeall("Can't open file");

    buf = AllocMem(11 * 512, MEMF_CHIP);
    if (buf == NULL)
        closeall("Can't allocate memory");
    ...
    while (count) {
        ...
        if (Write(file, buf, 512 * amount) != 512 * amount)
            goto err;
        first += amount;
    }
