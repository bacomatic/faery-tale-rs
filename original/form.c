...
struct NewWindow back_ground = {
    ...
    NOCAREREFRESH | ACTIVATE | SMART_REFRESH | WINDOWDEPTH | BORDERLESS |
        BACKDROP,
    NULL,
    NULL,
    NULL,
...
int open_all() {
    ...
    if (OpenDevice("console.device", 0L, (struct IORequest *)&ConsoleReq, 0L) != 0)
        return 5;
    ...
    if ((memstart = AllocMem(memsize, 0)) == NULL)
        return 7;
    SETFN(0x40);
    ...
    while (message = (struct IntuiMessage *)GetMsg(Window->UserPort)) {
        ...
        if (class == RAWKEY) {
            ...
            if (code == SELECTDOWN) {
                ...
                /*				if (pick_x == old_pick_x &&
                   pick_y == old_pick_y &&
                                                        DoubleClick(pick_secs,pick_mics,secs,mics)
                   ) clicktimes++; else clicktimes = 0; if (clicktimes == 3)
                   clicktimes = 0; old_pick_x = pick_x; old_pick_y = pick_y;
                                                cpos (pick_x, pick_y); */
...
int multistroke(short key)
{
    if (key >= 0x50 && key <= 0x59)
        return key + (140 - 0x50); /* funtions */
    if (key == 0x5f)
        return 128; /* help */
    if (key >= 0x4c && key <= 0x4f)
        return key + (130 - 0x4c); /* arrs */
    return 0;
}
...
char string_area[] = "(999)999-9999Test Field:  UUUTest2:   Fgd Bgd Box Ttl "
                     "Picture:Command:Color:R G B Size Title ";
...
struct edit_list main_form[] = {{&kitty, 20, 30},    {&puppy, 20, 20},
                                {&field10, 20, 45},  {&field11, 120, 45},
                                {&field12, 220, 45}, {&field13, 320, 45},
                                {&picfld, 20, 60}};

struct edit_form commnd = {38, 1, 26, 1, 62, 8, 0, 0, 5, 0, 0, 1};
struct edit_list step_form[] = {{&commnd, 20, 180}};

struct edit_form color_reg = {39, 2, 1, 2, 70, 6, 1, 0, 5, 0, 3, 1};
struct edit_list color_ask[] = {{&color_reg, 20, 180}};
...
void start_form(struct edit_list *form, short count, short start) {
    form_ptr = form;
    draw_form(count);
    edit_field(start, 0);
}

void draw_form(short n) {
    short i, x, y;
    struct edit_form *ef;
    index_b = -1;
    for (i = 0; i < n; i++) {
...
void edit_field(short n, short where) {
    current_field = n;
    if (n < 0 || n >= last_field) {
        ct_frm = NULL;
        end_form(form_ptr);
...
extern long atol();
long int_value(struct edit_form *ef) {
    char *buff, cs;
    long ival;
...
void clean_up_digits() {
    short i;
    float rval;
    if (ct_frm->edit_type & INTEGER)
        set_value(ct_frm, int_value(ct_frm));
    else if (ct_frm->edit_type & REAL) {
        ;
    }
}

void set_value(struct edit_form *ef, long val) {
    char *buff, cs;
    buff = buffer_area + ef->buffer;
    cs = buff[ef->buffer_length];
    if (ef->edit_type & PAD)
        ...
        sprintf(buff, "%*ld", ef->buffer_length, val);
    buff[ef->buffer_length] = cs;
}

void vset_contents(struct edit_form *ef, char *c) {
    short i;
    char *buff;
    buff = buffer_area + ef->buffer;
...
void change_fld(short p, char key) {
	/* key => key that got us out of last field */
    index_b = -1;
    clean_up_digits();
    print_field(ct_frm, ct_x, ct_y);
    if (p > 0)
    ...
    if (key == 8)
        edit_field(current_field, 1000);
    else
        edit_field(current_field, 0);
}

char last_key;
void edit_hook(char key) {
    char *buff;
    short i;
    last_key = key;
    if (ct_frm) {
        ...
        if (key == '\r') {
            ...
            if (index_b > 0) {
                index_b--;
                print_field(ct_frm, ct_x, ct_y);
            } else if ((ct_frm->edit_type & RETNEX) == 0)
                change_fld(-1, 8);
        } else if (key == 132) {
...
void mouse_hook(short mx, short my) {
    short i, x, y, l;
    struct edit_form *ef;
    index_b = -1;
    print_field(ct_frm, ct_x, ct_y);
...
void draw_title(struct edit_form *ef, short x, short y) {
    char *titl;
    ...
    SetBPen(rp, 0L);
    if (ef->title_length)
        Text(rp, titl, (long)(ef->title_length));
}

void print_box(struct edit_form *ef, short x, short y) {
    long x1, y1, x2, y2;
    if (ef->bordercolor) {
        x1 = x + (ef->title_length) * 8 - 1;
        y1 = y - 8;
        if (ef->picture_length)
            x2 = x1 + 2 + (ef->picture_length) * 8;
        else
            x2 = x1 + 2 + (ef->buffer_length) * 8;
        y2 = y + 2;
...
void print_field(struct edit_form *ef, short x, short y) {
    short i, b, fl;
    char *pict, *buff;
    buff = buffer_area + ef->buffer;
...
void end_form(struct edit_list *form) {
    char c;
    static long color;
    if (form == form_ask) {
        start_form(step_form, 1, 0);
    } else if (form == color_set) {
        SetRGB4(vp, color, int_value(&red_ask), int_value(&green_ask),
                int_value(&blue_ask));
        start_form(step_form, 1, 0);
        ...
        case 'C':
            cls();
            start_form(color_ask, 1, 0);
            break;
        case 'Q':
            close_all();
        case 'F':
            cls();
            start_form(form_ask, 1, 0);
            break;
        default:
            start_form(step_form, 1, 0); /* what a wierd loop!! */
        }
        ...
        start_form(step_form, 1, 0); /* if nothing else, start a command */
}
