...
struct Library *IntuitionBase, *GfxBase, *LayersBase, *DiskfontBase,
    *OpenLibrary();
...
struct IntuiText
samp_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Bold", NULL,
},
voxx_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Gfx Test", NULL,
},
text_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Repeat Test", NULL,
},
imge_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Test", NULL,
},
quit_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Quit", NULL,
};
...
struct Menu menu1 = {NULL,      1,       0, 56, 10, MENUENABLED | MIDRAWN,
                     "Control", &quit_mi};

struct IntuiText
lowr_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Low Resolution", NULL,
},
medr_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Hi Resolution", NULL,
},
hirs_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Interlaced", NULL,
};

struct MenuItem lowr_mi = {NULL, 0, 00, 140, 10, CF2, 0xfffb, (APTR)&lowr_tx},
                medr_mi = {&lowr_mi, 0,   10,     140,
                           10,       DF2, 0xfffd, (APTR)&medr_tx},
                hirs_mi = {&medr_mi, 0,   20,     140,
                           10,       DF2, 0xfffe, (APTR)&hirs_tx};

struct Menu menu2 = {&menu1,   70,      0, 44, 10, MENUENABLED | MIDRAWN,
                     "Screen", &hirs_mi};

struct IntuiText
load_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Load Item", NULL,
},
save_tx = {
    0, 1, JAM1, 18, 1, NULL, (UBYTE *)"Save Item", NULL,
};

struct MenuItem load_mi = {NULL, 0, 00, 120, 10, DF0, 0, (APTR)&load_tx},
                save_mi = {&load_mi, 0, 10, 120, 10, DF0, 0, (APTR)&save_tx};

struct Menu menu3 = {&menu2, 130,     0, 34, 10, MENUENABLED | MIDRAWN,
                     "File", &save_mi};

struct IntuiText
bold_tx = {
    0, 1, JAM1, 5, 1, NULL, (UBYTE *)"Bold", NULL,
},
ital_tx = {
    0, 1, JAM1, 5, 1, NULL, (UBYTE *)"Italic", NULL,
},
undr_tx = {
    0, 1, JAM1, 5, 1, NULL, (UBYTE *)"Underline", NULL,
},
font_tx = {
    0, 1, JAM1, 5, 1, NULL, (UBYTE *)"Load Font Dir", NULL,
};

struct MenuItem font_mi = {NULL, 0, 35, 100, 10, DF0, 0, (APTR)&font_tx},
...
struct Menu menu4 = {&menu3,  170,     0, 40, 10, MENUENABLED | MIDRAWN,
                     "Style", &undr_mi};

/* temporary open requester */

struct IntuiText open_tx = {
    0, 1, JAM1, 10, 3, NULL, (UBYTE *)"Open File", NULL,
};
struct IntuiText ok_tx = {
    0, 1, JAM1, 3, 2, NULL, (UBYTE *)"Open", NULL,
};
struct IntuiText nix_tx = {
    0, 1, JAM1, 3, 2, NULL, (UBYTE *)"Cancel", NULL,
};
...
struct Gadget nix_click = {&ok_click,
                           70,
                           22,
                           50,
                           12,
...
struct StringInfo int_i = {
    int_buf, NULL, 0, 3, 0,
};
struct Gadget set_num = {
    &nix_click,   143,       3,       20,
    16,           GADGHCOMP, LONGINT, STRGADGET | REQGADGET,
    NULL,         NULL,      NULL,    NULL,
    (APTR)&int_i, NULL,      NULL,
};

struct StringInfo string_i = {
    string_buf, NULL, 0, 40, 0,
};
...
struct Requester open_req = {NULL, 20, 30,         180,       36,
                             0,    0,  &file_name, &req_bord, &open_tx,
                             0,    2,  NULL,       NULL};
            ...
            if (class == MOUSEMOVE)
                ...
                if (code != MENUNULL) {
                    if (MENUNUM(code) == 3) {
                        switch (ITEMNUM(code)) {
                        case 4:
                            SetSoftStyle(rp, FSF_ITALIC, -1);
                            break;
                        case 3:
                            SetSoftStyle(rp, FSF_BOLD, -1);
                            break;
                        case 2:
                        ...
                        case 0: /* save */
                            ...
                            case VOICE_MODE:
                            case WAVE_MODE :
                                open_file_req("Save Voices",SAVE_VOICE);
                                break;
                            case SCORE_MODE:
                                open_file_req("Save Score",SAVE_SCORE);
                                break;
                            }
                            */
                            break;
                        case 1: /* load */
                            ...
                            case WAVE_MODE:
                            case VOICE_MODE:
                                open_file_req("Load Voices", LOAD_VOICE);
                                break;
                            case SCORE_MODE:
                                open_file_req("Load Score", LOAD_SCORE);
                                break;
                            case SAMPLE_MODE:
                                open_file_req("Load Samples", LOAD_SAMPLE);
                                break;
                            }
                            */
                            break;
...
void open_file_req(char *name, short mode) {
    req_mode = mode;
...
void open_screen(BYTE r) {
    ...
    if ((Window = OpenWindow(&back_ground)) == NULL)
        return;
...
void getfonts() {
    ...
    if (afh) {
        ...
        */
        ...
        if (fontnames == NULL)
            goto fail;
        AvailFonts((STRPTR)afh, AFTSIZE, 0xff);
...
void closefonts() {
    /*
    if (fontmenus)
        FreeMem(fontmenus,MNSIZE * 20);
    */
    if (fontnames)
        FreeMem(fontnames, ITSIZE * 20);
}
