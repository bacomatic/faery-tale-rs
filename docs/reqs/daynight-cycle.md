...
| ID | Requirement |
|----|-------------|
| R-CLOCK-001 | `daynight` shall be a 16-bit unsigned counter cycling from 0 to 23999, incrementing by 1 per non-scrolling world-clock tick. Full cycle = 24000 world-clock ticks, distinct from both the 30 fps presentation cadence and the 15 Hz animation/AI tick. |
...
| R-CLOCK-005 | During sleep: `daynight += 63` per world-clock tick (plus normal +1 = 64 effective advance per world-clock tick). |
...
| R-CLOCK-007 | Outdoor palette shall be faded via `day_fade()` every 4 world-clock ticks (`(daynight & 3) == 0`) or during screen rebuild (`viewstatus > 97`), using `lightlevel` to scale RGB components with clamping (red min 10 max 100, green min 25 max 100, blue min 60 max 100). |
