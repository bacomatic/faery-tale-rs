...
| R-COMBAT-020 | Goodfairy countdown: 255→200 death sequence; 200→120 luck gate (luck < 1 → `revive(TRUE)` brother succession, FALL → `revive(FALSE)` non-lethal); 120→20 fairy sprite flies toward hero; 19→1 fairy stationary at `hero_x + 20`; 1 → `revive(FALSE)` fairy rescue. |
