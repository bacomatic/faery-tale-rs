...
use songs::{
    SongLibrary, Track, TrackEvent,
    PTABLE, NOTE_DURATIONS, AMIGA_CLOCK_NTSC, VBL_RATE_HZ, DEFAULT_TEMPO,
};
...
impl Voice {
    fn new() -> Self {
        Voice {
            ...
            lp_state: 0.0, declick: 0.0,
            event_start: 0, event_stop: 0,
            trak_ptr: None, trak_beg: None,
        }
    }
    ...
    fn trigger_note(&mut self, pitch: usize, inst: &Instruments) {
        ...
        self.wave_len   = ((32 - wave_offset as usize) * 2)
            .min(WAVEFORM_BYTES - self.wave_start);
    ...
    fn step_envelope(&mut self, envelopes: &[[u8; ENVELOPE_BYTES]; ENVELOPE_COUNT]) {
        ...
        let byte = envelopes
            [self.vol_num.min(ENVELOPE_COUNT - 1)]
            [self.vol_list.min(ENVELOPE_BYTES - 1)];
        if byte >= 0x80 {
            self.vol_delay = 0xff;
        } else {
...
impl SeqState {
    fn new() -> Self {
        SeqState {
            ...
            timeclock: 0, tempo: DEFAULT_TEMPO, nosound: true,
            tracks: [None, None, None, None],
            samples_to_vbl: 0.0, muted: [false; 4],
        }
    }
    ...
    fn play_score(&mut self, group_tracks: [Arc<Track>; 4], inst: &Instruments) {
        ...
        for (i, v) in self.voices.iter_mut().enumerate() {
            *v = Voice::new();
            ...
            let first    = inst.envelopes[vol_num][0];
            if first < 0x80 { v.volume = first.min(64); v.vol_list = 1; }
            else             { v.vol_list = 0; }
    ...
    fn tick_voice(&mut self, vi: usize, inst: &Instruments) {
        ...
        loop {
            let event = match track.get(ptr) { Some(e) => e, None => break };
            ptr += 1;
            match event {
                ...
                TrackEvent::End { looping } => {
                    if *looping { ptr = self.voices[vi].trak_beg.unwrap_or(0); }
                    else        { self.voices[vi].trak_ptr = None; self.voices[vi].silence(); return; }
                }
                TrackEvent::Unknown { .. } => {}
            }
        }
...
impl AudioCallback<i16> for SynthCallback {
    fn callback(&mut self, stream: &mut sdl3::audio::AudioStream, requested: i32) {
        ...
        let mut st = match self.state.lock() { Ok(g) => g, Err(_) => { let _ = stream.put_data_i16(&out); return; } };
        ...
        while frame_pos < total_frames {
            ...
            let chunk = (st.samples_to_vbl.floor() as usize)
                .min(total_frames - frame_pos).max(1);
            let mut left_buf  = vec![0.0f32; chunk];
            let mut right_buf = vec![0.0f32; chunk];
            let muted = st.muted;
            // Paula: ch0, ch3 → Left; ch1, ch2 → Right
            st.voices[0].mix_stereo(&mut left_buf,  &mut right_buf, inst, STEREO_PRIMARY, STEREO_BLEED, muted[0]);
            st.voices[3].mix_stereo(&mut left_buf,  &mut right_buf, inst, STEREO_PRIMARY, STEREO_BLEED, muted[3]);
            st.voices[1].mix_stereo(&mut right_buf, &mut left_buf,  inst, STEREO_PRIMARY, STEREO_BLEED, muted[1]);
            st.voices[2].mix_stereo(&mut right_buf, &mut left_buf,  inst, STEREO_PRIMARY, STEREO_BLEED, muted[2]);
            for i in 0..chunk {
...
fn pitch_name(pitch: u8) -> String {
    if pitch as usize >= PTABLE.len() { return "?".to_string(); }
    let (period, _) = PTABLE[pitch as usize];
    if period == 0 { return "---".to_string(); }
...
impl TrackVis {
    fn new(track: &Track) -> Self {
            ...
            match event {
                TrackEvent::Note { pitch, duration_idx } => {
                    ...
                    events.push(ScheduledEvent {
                        tick_start: tick, tick_dur: dur,
                        label: pitch_name(*pitch), is_rest: false,
                    });
                    tick += dur;
                }
                TrackEvent::Rest { duration_idx } => {
                    ...
                    events.push(ScheduledEvent {
                        tick_start: tick, tick_dur: dur,
                        label: String::new(), is_rest: true,
                    });
                    tick += dur;
                }
                TrackEvent::SetInstrument { slot } => { instrument = *slot; }
                TrackEvent::SetTempo { .. } => {}
                TrackEvent::End { looping } => {
                    if *looping { loop_len = tick; }
                    break;
                }
                TrackEvent::Unknown { .. } => {}
            }
        ...
        let min_note_dur = events.iter()
            .map(|e| e.tick_dur)
            .filter(|&d| d > 0)
            .min()
            .unwrap_or(u32::MAX);

        TrackVis { events, loop_len, min_note_dur, instrument, enabled: true }
    }
    ...
    fn event_at(&self, row_tick: u32, ticks_per_row: u32) -> Option<(&ScheduledEvent, bool)> {
        ...
        let eff      = if self.loop_len > 0 { row_tick % self.loop_len } else { row_tick };
...
impl App {
    fn new(songs: SongLibrary, instruments: Instruments, group: usize,
           audio: Arc<Mutex<SeqState>>) -> Self
    {
        let tracks        = Self::make_tracks(&songs, group);
        let ticks_per_row = Self::auto_scale(&tracks);
        App { songs, instruments, current_group: group, tracks, ticks_per_row, paused: false, audio }
    }
    ...
    fn auto_scale(tracks: &[TrackVis; 4]) -> u32 {
        ...
        while tpr < min_dur { tpr *= 2; }
        tpr
    }

    fn switch_group(&mut self, group: usize) {
        if self.songs.group(group).is_none() { return; }
...
fn instrument_name(slot: u8) -> &'static str {
    INSTRUMENT_NAMES.get(slot as usize).copied().unwrap_or("Unknown")
}
...
fn render(stdout: &mut io::Stdout, app: &App, timeclock: u32, tempo: u32) -> io::Result<()> {
    ...
    for display_row in 0..grid_rows {
        ...
        let row_tick    = if row_idx < 0 {
            ...
            execute!(stdout, Print(pad_or_clip(&line, term_w)), cursor::MoveToNextLine(1))?;
            continue;
        } else {
        ...
        if is_center {
            ...
            let color = if dist <= 2 {
                if is_future { Color::White } else { Color::Grey }
            } else if dist <= 8 {
                Color::Grey
            } else {
                Color::DarkGrey
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let device = audio_ss.open_playback_stream(&spec, SynthCallback { state: cb_state, instruments: cb_inst })?;
    device.resume()?;

    // Start playback for the initial group
    {
        let group_tracks = songs.group(initial_group)
        ...
        while event::poll(Duration::from_millis(0))? {
            if let Event::Key(KeyEvent { code, modifiers, .. }) = event::read()? {
                match code {
                    ...
                    KeyCode::Char('1') => { app.toggle_track(0); }
                    KeyCode::Char('2') => { app.toggle_track(1); }
                    KeyCode::Char('3') => { app.toggle_track(2); }
                    KeyCode::Char('4') => { app.toggle_track(3); }

                    KeyCode::Char('[') | KeyCode::Char(',') => { app.zoom_in(); }
                    KeyCode::Char(']') | KeyCode::Char('.') => { app.zoom_out(); }
