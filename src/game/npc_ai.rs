...
pub fn do_tactic(
    ...
    match npc.tactic {
        ...
        Tactic::BumbleSeek => {
            set_course(npc, hero_x, hero_y, SetCourseMode::Bumble);
        }
        Tactic::Backup => {
            set_course(npc, hero_x, hero_y, SetCourseMode::Flee);
        }
        Tactic::Follow => {
        ...
        Tactic::Evade => {
            if let Some(li) = leader_idx {
                ...
                if neighbor < npcs.len() {
                    ...
                    set_course(npc, nx, ny + 20, SetCourseMode::Deviate2);
                }
            }
        }
        Tactic::EggSeek => {
            set_course(npc, 23087, 5667, SetCourseMode::Smart);
            npc.state = NpcState::Walking;
        }
...
mod tests {
    ...
    fn test_set_course_same_position() {
        // Same position → state should be Still (com2 returns Direction::None).
        let mut npc = make_npc(100, 100);
        set_course(&mut npc, 100, 100, SetCourseMode::Smart);
