...
use crate::game::magic::{
    use_magic, ITEM_LANTERN, ITEM_ORB, ITEM_RING, ITEM_SKULL, ITEM_STONE_RING, ITEM_TOTEM,
    ITEM_VIAL,
};
...
impl GameplayScene {
    pub fn new() -> Self {
        GameplayScene {
            ...
            textcolors: [0u32; 32],
            sprite_sheets: (0..crate::game::sprites::CFILE_COUNT)
                .map(|_| None)
                .collect(),
            object_sprites: None,
    ...
    pub fn init_from_library(&mut self, game_lib: &GameLibrary) {
        if let Some(bro) = game_lib.get_brother(0) {
            ...
            self.state
                .init_first_brother(bro.brave, bro.luck, bro.kind, bro.wealth, sx, sy, sr);
        }
        ...
        self.doors = game_lib
            .doors
            .iter()
            .map(|d| crate::game::doors::DoorEntry {
        ...
        match self.state.brother {
            1 => {
                msg9.push('.');
                self.messages.push_wrapped(msg9);
            }
            2 => {
                self.messages.push_wrapped(msg9);
                self.messages
                    .push_wrapped(crate::game::events::event_msg(&self.narr, 10, &bname));
            }
            _ => {
                self.messages.push_wrapped(msg9);
                self.messages
                    .push_wrapped(crate::game::events::event_msg(&self.narr, 11, &bname));
            }
        }
    ...
    pub fn is_witch_active(&self) -> bool {
        self.witch_effect.active
    }

    /// Whether the teleport flash/fade effect is active.
    pub fn is_teleport_active(&self) -> bool {
        self.teleport_effect.active
    }

    /// Whether a palette crossfade (region transition) is in progress.
    pub fn is_palette_xfade_active(&self) -> bool {
        self.palette_transition.is_some()
    }
